...
from generate_certificate import generate_certificate  # Import the fixed module
...
def wipe_file_route():
    ...
    if not os.path.exists(C_EXECUTABLE_PATH):
        return jsonify({'stderr': f"ERROR: Executable not found."}), 500
    ...
    try:
        cert_json, cert_pdf = generate_certificate("wipe.log")
    except Exception as e:
        return jsonify({'stderr': f"Certificate generation failed: {str(e)}"}), 500

    return jsonify({'log': log_output, 'success': True, 'certificate_json': cert_json, 'certificate_pdf': cert_pdf})
...
if __name__ == '__main__':
    if not os.path.exists('users.db'):
        print("ERROR: Database 'users.db' not found! Run 'python database.py' first.")
    else:
        print("Starting Zero Leaks server at http://127.0.0.1:5000")
...
from werkzeug.security import generate_password_hash, check_password_hash
from functools import wraps
from generate_certificate import generate_certificate  # Fixed module
app = Flask(__name__)
app.secret_key = os.urandom(24)
...
def send_otp():
    otp = str(random.randint(100000, 999999))
    session['otp'] = otp
    print(f"OTP for {session.get('username')}: {otp}")
    flash("OTP sent to console.", "info")
    return redirect(url_for('verify_otp'))
...
def wipe_file_route():
    ...
    # Generate certificate after wiping
    try:
        cert_json, cert_pdf = generate_certificate("wipe.log")
    except Exception as e:
        return jsonify({'stderr': f"Certificate generation failed: {str(e)}"}), 500
